...
describe('resolveDocumentTitle', () => {
  ...
  it('站点名为空时，回退默认站点名', () => {
    expect(resolveDocumentTitle('Dashboard', '')).toBe('Dashboard - 澄川 AI')
    expect(resolveDocumentTitle(undefined, '   ')).toBe('澄川 AI')
    expect(resolveDocumentTitle('Dashboard', 'Sub2API')).toBe('Dashboard - 澄川 AI')
  })
