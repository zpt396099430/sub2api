export default {
  ...
  globalPricing: {
      modeToken: "按词元计费",
      modePerRequest: "按请求计费",
      modeImage: "按图片计费",
      modeVideo: "按视频计费",
      title: '全站模型定价',
      description: '命中模型的请求在全部分组/账号按此价格计费；未设置的模型保持原定价。改价最多 60 秒生效。',
      addPricing: '新增定价',
      editPricing: '编辑定价',
      deletePricing: '删除定价',
      modelPattern: '模型',
      modelPatternPlaceholder: '如 gpt-5 或 gpt-5*',
      modelPatternHint: '精确模型名（不区分大小写）或单个末尾 * 通配。',
      billingMode: '计费模式',
      inputPrice: '输入价格（美元 / 百万词元）',
      outputPrice: '输出价格（美元 / 百万词元）',
      cacheReadPrice: '缓存读取（美元 / 百万词元）',
      cacheWritePrice: '缓存写入 5 分钟（美元 / 百万词元）',
      cacheWrite1hPrice: '缓存写入 1 小时（美元 / 百万词元）',
      perRequestPrice: '按次价格（美元）',
      enabled: '启用',
      source: '生效来源',
      sourceGlobal: '全站覆盖',
      noPricing: '暂无全站定价，命中模型将在全站使用此处价格。',
      confirmDelete: '删除该覆盖？对应模型将恢复原定价。',
      createSuccess: '全站定价已创建',
      updateSuccess: '全站定价已更新',
      deleteSuccess: '全站定价已删除',
      loadFailed: '加载全站定价失败',
      saveFailed: '保存全站定价失败',
      deleteFailed: '删除全站定价失败',
      invalidPrice: '价格必须是非负数字',
      patternRequired: '请填写模型',
      optional: '选填',
    },
  ...
  margin: {
      ...
      actionFused: "已熔断",
      actionRecovered: "已恢复",
      reasonLowMargin: "毛利率低于阈值",
      reasonCooldown: "冷却结束，恢复试探",
      reasonManual: "手动恢复",
      title: '毛利中心',
      description: '收入为用户实扣，成本按目录价估算；持续亏损的渠道会被自动熔断摘除。',
      channel: '渠道',
      model: '模型',
      requests: '请求数',
      revenue: '收入',
      ...
      windowHours: '统计窗口（小时）',
      minSpend: '最低预估支出（$）',
      minMarginRate: '熔断毛利率阈值',
      cooldownHours: '冷却时长（小时）',
      intervalMinutes: '检查间隔（分钟）',
      fuseHint: '仅评估支出达标的渠道；熔断渠道冷却后自动恢复试探。',
      noData: '该窗口暂无计费流量。',
      loadFailed: '加载毛利数据失败',
      saveFailed: '保存失败',
      saveSuccess: '保存成功',
  ...
  tieredRouting: {
      title: '分级路由',
      description: '高级用户优先使用优选池账号，其他用户使用全部账号。请先在账号编辑页标记账号池。',
      enable: '启用分级路由',
      premiumUserIds: '高级用户编号',
      premiumUserIdsPlaceholder: '如 12, 34, 56',
      premiumUserIdsHint: '逗号或空格分隔的用户编号，这些用户优先使用优选池。',
      minBalance: '高级用户最低余额（美元）',
      minBalanceHint: '余额达到该值的用户视为高级用户，0 表示关闭余额规则。',
      poolHint: '在账号编辑页（代理区域）把账号标记为 优选池；优选池无可用时回退全部账号。',
      loadFailed: '加载分级路由设置失败',
      saveFailed: '保存分级路由设置失败',
      saveSuccess: '分级路由设置已保存',
      invalidBalance: '最低余额必须是非负数字',
    },

  spendGuard: {
      reasonTokenRate: "词元消耗速率超过阈值",
      reasonErrorRate: "错误率超过阈值",
      reasonManual: "手动解冻",
      title: '烧钱防护',
      description: '词元消耗速率或错误率异常的密钥 会被自动冻结，人工确认后手动解冻。',
      settings: '防护规则',
      enableAuto: '自动冻结异常密钥',
      windowMinutes: '统计窗口（分钟）',
      tokensPerMinute: '速率阈值（词元 / 分钟）',
      minRequests: '最少请求数',
      maxErrorRate: '错误率阈值（0-1）',
      intervalSeconds: '检查间隔（秒）',
      settingsHint: '请求数不足的密钥 永不冻结；被冻结的密钥 保持禁用直到手动解冻。',
      key: 'API 密钥',
      requests: '请求数',
      tokensPerMin: '词元 / 分钟',
      errRate: '错误率',
      ...
      statusExpired: '已过期',
      unfreeze: '解冻',
      unfreezeSuccess: '密钥已解冻',
      unfreezeFailed: '解冻失败',
      events: '冻结事件',
      noData: '窗口内暂无密钥活动。',
      loadFailed: '加载烧钱防护数据失败',
      saveFailed: '保存失败',
      saveSuccess: '保存成功',
    },
