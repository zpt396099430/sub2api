...
func (s *OpenAIGatewayService) forwardGrokMediaVideoContent(
	...
	upstreamStart := time.Now()
	statusResp, err := s.httpUpstream.Do(WithAccountTrafficRequest(statusReq, account), proxyURL, account.ID, account.Concurrency)
	if err != nil {
		SetOpsLatencyMs(c, OpsUpstreamLatencyMsKey, time.Since(upstreamStart).Milliseconds())
		return nil, s.handleOpenAIUpstreamTransportError(ctx, c, account, err, false)
	}
	...
	contentResp, err := s.httpUpstream.Do(WithAccountTrafficRequest(contentReq, account), proxyURL, account.ID, account.Concurrency)
	SetOpsLatencyMs(c, OpsUpstreamLatencyMsKey, time.Since(upstreamStart).Milliseconds())
	if err != nil {
		return nil, s.handleOpenAIUpstreamTransportError(ctx, c, account, err, false)
	}
