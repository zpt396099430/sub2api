...
func TestGrokMediaGenerationEligibility(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "inconclusive successful billing requires paid entitlement evidence", account: &Account{Platform: PlatformGrok, Type: AccountTypeOAuth, Extra: map[string]any{grokBillingExtraKey: &xai.BillingSummary{StatusCode: http.StatusOK, Partial: true}}}, want: false, wantReason: "billing_inconclusive"},
